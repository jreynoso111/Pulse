...
- The React SPA is namespaced under `#/app/*` (hash routing).
...
## Bootstrap Behavior
- `src/main.jsx` mounts React when `window.location.hash` starts with `#/app`.
- `#marketing-landing` is visible for non-`#/app` routes.
- `#root` is hidden by default and shown only for `#/app` routes.

## Navigation Consistency
- Sidebar links in `src/components/Sidebar.jsx` must stay aligned with `#/app/*` routes (using React Router `to="/app/..."` inside `HashRouter`).
